    ...
    <el-table :data="tableData" border style="width: 100%;margin-top: 20px;"
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="correction(scope.row)">批改</el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <div class="questionClass">
          <div class="testPaperHeader">
            <div style="font-size:20px;">{{testPaper.name}}
              <el-button type="primary" @click="submit()" style="float:right;margin-top: 10px;margin-left:20px;">批改完成
              </el-button>
              <el-button style="float:right;margin-top: 10px;" @click="dialogFormVisible=false">关闭
              </el-button>
            </div>
          ...
          <div v-for="(question,index) in testPaper.questionList" class="questionClass2">
            ...
            <div style="margin-top:20px;margin-bottom:20px;color: red;">
              得分：
              <el-input-number v-model="question.correctScore" :min="0" :max="question.score"></el-input-number>
            </div>
          </div>
        </div>
      </el-dialog>
...
export default {
  methods: {
    ...
    getDataPage(currentPage, pageSize) {
      ...
      that.$http
        .get('/examinationManage/getTestPaperCorrectionPage', {
          params: param
        })
        .then(function (response) {
          if (response.data.code == 200) {
    ...
    submit() {
      const that = this;
      that.$http
        .post('/examinationManage/submitCorrect', {
          "joinid": that.curJoinId,
